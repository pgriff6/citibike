...
The objective function has three constraints: first, the distance between each candidate station and a user's intended station must not exceed the maximum distance $r_u$ the user is willing to walk. Second, the budget available to each user in the network at time t is constraint by the total budget $B$ and the number of users in the system $n$. Third, in order to make practical sense, all definitions in the optimization problem must be positive except for the user dissatisfaction function $D_u$ and the congestion level function $G_s$.
\begin{equation}
d_{pu*} \leq r_u
\end{equation}
\begin{equation}
